...
- **自訂點重複渲染修正（07-06 晚，使用者回報「三個內湖」）**：主卡選自訂點時，進階點卡列會把「HERE 卡」和「自訂清單卡」畫成兩張同一點——修為 HERE 卡只在 `LOC.type==="geo"`（目前定位）時出現，主卡選定的自訂卡改標「· 主卡」。同名區辨：所有點卡加 `.psub` 副標（固定點=地標名、自訂點=綁定測站名、HERE=最近站名），管理面板路徑選單自訂點加「（自訂）」後綴。SW rain-v17。
...
### PATCH-2026-07 實施記錄（07-22，鄰站進 fusion；詳見 PATCH-2026-07-promote-neighbor.md）
- **已實施**：`buildNowcast` 加 `nb` 參數（乾站無 QPF 時 nb≥5→tier2「鄰區在下，可能移入」／nb2–5→tier1「留意移入」，claim=1h 進帳）；舊 nearbyHint/omHint 提示層與 OM 參考行移除（畫面全 CWA）；GATE 加 NB_MID=2/NB_HIGH=5。SW rain-v18。
- **全期回測（18,352 筆真實重演）**：誤報 +0.018（≤預估 0.05 ✓）、漏報 0.030→0.028 ✓、舊 461 筆漏報 71% 至少獲「留意」✓、7/6 永和個案完美重演 ✓。**抓到一個高估**：增量筆實際下雨率 .255/.084（非邊際關聯的 .44）→ 可能性對映下修為 中/低（教訓入 SPEC §8：升級前必做增量回測）。
- **同場裁決**：OM 進 fusion 出局（z=24.6，降級幕後健康監測）、寬 QPF 負結果收案（z=16.2）、JMA 留備援候選。影子欄位全數續記。
- **待驗**：部署後下份週報 `scores_1h.miss` 不升、`false_alarm` 升幅 ≤0.05；找一場移入型雨看「鄰區在下」實戰。
