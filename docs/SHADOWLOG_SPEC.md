...
2. 影子實驗一律「先記錄、不進 fusion、對答案、人工拍板」。裁決時點不綁固定週數：
   **關鍵指標 95%CI 半寬 < 0.03 且方向明確即可結案**（2026-07-22 修訂，原為固定四週）。
   升級前必做**增量回測**：邊際關聯 ≠ 增量預測力（教訓：鄰站 th5 邊際 .44 vs 增量 .255）。
3. 資料源去留是實驗問題，不是信仰問題。
4. QPF 時效守衛：`qpf.json` 超過 70 分鐘視同無 QPF（避免舊雨帶冒充「未來 1h」）。
5. **Type-II 文案鐵律（2026-08-03 使用者拍板）**：任一源（QPF/鄰站/plan3/特報）有訊號 →
   文案至少帶傘級；「放心出門」僅限全源靜默。偽陽性是修辭問題可容忍，有訊號卻無警示不可發生。
6. **QPF 校準 gating（2026-08-03，docs/PATCH-2026-08）**：q≥10 或 rising+地面雨跡才喊雨；
   弱 QPF 降級 tier1「可能有短暫雨」。校準表 regime 漂移為結構性事實（同桶雨週 .33／乾週 .07），
   靜態門檻站在漂移最小的 10–20/20+ 桶上；滾動表 gating＝自動調門檻，未獲修訂前不採。
